...
export class ReclamLibroComponent implements OnInit {
  ...
  constructor(private PresentarReclamService: PresentarReclamService, private router: Router,
    private Swal2Service: Swal2Service, private PideService: PideService) { }
...
/* -------------------------------------------------------------------------------------------------------- */
  searchDni() {
    if (
      ...
      this.PideService.searchDniExterno(
        ...
        (resp: any) => {
          console.log(resp);
          if(Number(resp.codVerifica) ==  Number(this.FormReclamLibro.value.nroVerifDNI)){
            ...
            this.FormReclamLibro.controls['apPaterno'].setValue(resp.apellidoPaterno),
            this.FormReclamLibro.controls['apMaterno'].setValue(resp.apellidoMaterno);
      ...
      this.PideService.searchRuc(
        ...
        (resp:any) => {
          ...
          if(Number(resp.ruc) == Number(this.FormReclamLibro.value.ndoc)){
            ...
            this.FormReclamLibro.controls['nombres'].setValue(this.FormReclamLibro.value.ape_nom.slice(14,28)),
            this.FormReclamLibro.controls['apPaterno'].setValue(this.FormReclamLibro.value.ape_nom.slice(0,6)),
            this.FormReclamLibro.controls['apMaterno'].setValue(this.FormReclamLibro.value.ape_nom.slice(7,13));
            ...
            this.Swal2Service.alertaToast('ATENCIÓN', 'Numero de RUC erróneo', 'warning');
            this.FormReclamLibro.controls['nombres'].setValue(null);
          }
        },
        (error:any) =>{
  ...
  PresentarReclamo() {
    ...
    if (this.FormReclamLibro.valid) {
      this.PresentarReclamService.PresentarReclam(this.FormReclamLibro.value).subscribe(
        (resp: any) => {
          console.log(resp);
          if (resp.validated) {
            console.log(1);
